...
export default createWidget('poster-name', {
  ...
  html(attrs) {
    ...
    if (((Discourse.User.current() && this.siteSettings.hide_full_names_without_login_enabled) || !(this.siteSettings.hide_full_names_without_login_enabled)) && name && this.siteSettings.display_name_on_posts && sanitizeName(name) !== sanitizeName(username)) {
